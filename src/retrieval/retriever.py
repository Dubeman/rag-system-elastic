...
class HybridRetriever:
    ...
    def search_hybrid(self, query: str, top_k: int = 5, mode: str = "dense_bm25") -> List[Dict]:
        """Parallel hybrid search for 3-5x speed improvement."""
        ...
        if mode == "bm25_only":
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                futures = [
                    executor.submit(self.search_bm25, query, top_k),
                    executor.submit(self.search_dense, query, top_k)
                ]
                
                bm25_results, dense_results = [f.result() for f in futures]
                
                all_results = [r for r in [bm25_results, dense_results] if r]
                if len(all_results) > 1:
                    return self.reciprocal_rank_fusion(all_results)[:top_k]
                return all_results[0][:top_k] if all_results else []
                
        elif mode == "full_hybrid":
            # Parallel: All three methods
            with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
                futures = [
                    executor.submit(self.search_bm25, query, top_k),
                    executor.submit(self.search_dense, query, top_k),
                    executor.submit(self.search_elser, query, top_k)
                ]
                
                # Wait for all to complete
                bm25_results, dense_results, elser_results = [f.result() for f in futures]
                
                all_results = [r for r in [bm25_results, dense_results, elser_results] if r]
                if len(all_results) > 1:
                    return self.reciprocal_rank_fusion(all_results)[:top_k]
                return all_results[0][:top_k] if all_results else []
